...
// Parameters: no parameters
// Returns: a Promise that resolve to an elliptic curve key using P-384 with cipher key derivation privileges. Allows for deriving AES GCM 256 cipher key
//  -- 
...
async function galactic_user_starter_keys(selector) {
	...
	if ( (selector === undefined) || (selector === "derived") ) {
		// Generate a local public/private key pair SIGNER
...
function derive_aes_key(sender_pub_key_buffer,local_private) {
	let derived_aes = await g_crypto.deriveKey(
	  {
		name: "ECDH",
		public: sender_pub_key_buffer
	  },
	  local_private,
	  {
		name: "AES-CBC",
...
// Returns: The clear string or false if it cannot be decrypted
*/
async function derived_decipher_message(message,sender_public,priv_key,nonce) {
	try {
		let aes_key = await derive_key(sender_public,priv_key)
		if ( aes_key ) {
			let iv_nonce = from_base64_to_uint8array(nonce)
			let buffer = from_base64_to_uint8array(message)
			let clear = await aes_decipher_message(buffer,aes_key,iv_nonce)
...
module.exports.pc_keypair_promise = pc_keypair_promise
module.exports.pc_wrapper_keypair_promise = pc_wrapper_keypair_promise
